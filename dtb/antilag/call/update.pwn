...
hook OnPlayerUpdate(playerid)
{
	if (GetTickCount() - previous_tick[playerid] > 1000)
	{
		...
		if (ping > MAX_PING && ping != INVALID_PING)
		{
			...
			{
				new reason[30];
				format(reason, sizeof(reason), "High Ping (%i ms)", ping);
		...
		else
		{
			...
			if (fps < MIN_FPS)
			{
				...
				{
					new reason[40];
					format(reason, sizeof(reason), "Low Framerate (%i FPS)", fps);
					KickPlayer(playerid, reason);
				}
		...
		if (packetloss > MAX_PACKETLOSS)
		{
			...
			{
				new reason[40];
				format(reason, sizeof(reason), "Packetloss (%.1f %%)", packetloss);
				KickPlayer(playerid, reason);
			}

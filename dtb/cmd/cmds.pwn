...
{
	new caption[50], info_part[100], info[1500];

	format(caption, sizeof caption, "{%06x}Server Commands:", RGBAtoRGB(COLOR_WHITE));
	...
	{
		format(
			info_part, sizeof info_part, "\n{%06x}Admin Level 1 (%s):{%06x}\n", RGBAtoRGB(COLOR_WHITE), GetAdminLevelName(1),
	...
	{
		format(
			info_part, sizeof info_part, "\n{%06x}Admin Level 2 (%s):{%06x}\n", RGBAtoRGB(COLOR_WHITE), GetAdminLevelName(2),
			RGBAtoRGB(COLOR_ORANGE));
		strcat(info, info_part);
	...
	ShowPlayerDialog(playerid, DIALOG_CMDS, DIALOG_STYLE_MSGBOX, caption, info, "Close", "");
	return 1;
}

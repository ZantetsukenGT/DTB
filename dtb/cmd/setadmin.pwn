COMMAND:setadmin(playerid, params[])
{
	...
	mysql_format(
		...
		g_PlayerAccountID[id], g_PlayerAccountID[playerid], level);
	mysql_tquery(g_DBHandle, query);

	format(
		message, sizeof(message), "You have set the admin level of %s (ID: %i) to level %i (%s).", g_PlayerName[id], id, level,
		GetAdminLevelName(level));
	SendClientMessage(playerid, COLOR_ORANGE, message);

	format(
		message, sizeof(message), "Your admin level has been set to level %i (%s) by %s (ID: %i).", level,
		GetAdminLevelName(level), g_PlayerName[playerid], playerid);
	SendClientMessage(id, COLOR_ORANGE, message);

	format(
		message, sizeof(message), "%s (ID: %i) has set the admin level of %s (ID: %i) to level %i (%s).", g_PlayerName[playerid],
		playerid, g_PlayerName[id], id, level, GetAdminLevelName(level));

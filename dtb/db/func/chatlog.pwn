...
{
	new message_len = strlen(message), query[400];
	...
	if (to_playerid == INVALID_PLAYER_ID)
	{
		mysql_format(
			...
			g_PlayerAccountID[by_playerid], g_PlayerName[by_playerid], message);
	}
	else
	{
		mysql_format(
			...
			g_PlayerAccountID[by_playerid], g_PlayerAccountID[to_playerid], g_PlayerName[by_playerid], g_PlayerName[to_playerid],
			message);
	}

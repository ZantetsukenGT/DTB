IRCCMD:kick(botid, const channel[], const user[], const host[], const params[])
{
	if (botid == g_IRCBotID[0])
	{
		...
		if (sscanf(params, "rS(None)[*]", playerid, sizeof(reason), reason))
			return IRC_GroupSay(g_IRCGroupID, IRC_CHANNEL, "4,1SYNTAX: !Kick <playerid/name> <reason>"), 1;

		if (playerid == INVALID_PLAYER_ID)
			return IRC_GroupSay(g_IRCGroupID, IRC_CHANNEL, "4,1ERROR: This player is not connected!"), 1;

		KickPlayer(playerid, reason, INVALID_PLAYER_ID, user);
	}
	return 1;
}

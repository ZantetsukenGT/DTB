...
hook OnGameEnd(reason)
{
	foreach (new playerid : Player)
	{
		switch (reason)
		{
				...
				switch (g_PlayerTeam[playerid])
				{
					...
					default:
						PlayerTextDrawSetString(
							playerid, g_pScoreboardResultTD[playerid], "~r~attackers won~n~~w~target destroyed");
				}
			}
			case REASON_DEFUSED:
			{
			...
			case REASON_ATTACK_KILLED:
			{
				switch (g_PlayerTeam[playerid])
				{
					case TEAM_ATTACK: PlayerTextDrawSetString(playerid, g_pScoreboardResultTD[playerid], "~r~defeat~n~~w~killed");
					case TEAM_DEFEND:
						PlayerTextDrawSetString(playerid, g_pScoreboardResultTD[playerid], "~g~victory~n~~w~opponents killed");
					default:
						PlayerTextDrawSetString(
							playerid, g_pScoreboardResultTD[playerid], "~b~defenders won~n~~w~opponents killed");
				}
			}
			case REASON_DEFEND_KILLED:
			{
				switch (g_PlayerTeam[playerid])
				{
					case TEAM_ATTACK:
						PlayerTextDrawSetString(playerid, g_pScoreboardResultTD[playerid], "~g~victory~n~~w~opponents killed");
					case TEAM_DEFEND: PlayerTextDrawSetString(playerid, g_pScoreboardResultTD[playerid], "~r~defeat~n~~w~killed");
					default:
						PlayerTextDrawSetString(
							playerid, g_pScoreboardResultTD[playerid], "~r~attackers won~n~~w~opponents killed");
				}
			}
			case REASON_ATTACK_DISCONNECT:
			{
				switch (g_PlayerTeam[playerid])
				{
					case TEAM_DEFEND:
						PlayerTextDrawSetString(
							playerid, g_pScoreboardResultTD[playerid], "~g~victory~n~~w~opponents disconnected");
					default:
						PlayerTextDrawSetString(
							playerid, g_pScoreboardResultTD[playerid], "~b~defenders won~n~~w~opponents disconnected");
				}
			}
			case REASON_DEFEND_DISCONNECT:
			{
				switch (g_PlayerTeam[playerid])
				{
					...
					default:
						PlayerTextDrawSetString(
							playerid, g_pScoreboardResultTD[playerid], "~r~attackers won~n~~w~opponents disconnected");
				}
			}
			case REASON_NO_PLANT:
			{
				switch (g_PlayerTeam[playerid])
				{
					case TEAM_ATTACK:
						PlayerTextDrawSetString(
							playerid, g_pScoreboardResultTD[playerid], "~r~defeat~n~~w~no target was destroyed");
					case TEAM_DEFEND:
						PlayerTextDrawSetString(playerid, g_pScoreboardResultTD[playerid], "~g~victory~n~~w~targets defended");
					default:
						PlayerTextDrawSetString(
							playerid, g_pScoreboardResultTD[playerid], "~b~defenders won~n~~w~no target was destroyed");
				}
			}
	...
	for (new column; column < 4; column++) TextDrawShowForAll(g_ScoreboardColumnTD[column]);
	...
	foreach (new ref_playerid : Player)
	{
